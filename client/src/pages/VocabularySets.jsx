        ...
        {sets && sets.map(s => (
          <div className="vocab-set-card" key={s.id} onClick={() => navigate(`/vocabulary/set/${s.id}`)}>
            ...
            <div className="vocab-set-card-meta">
              <span>🔤 {s.word_count} word{s.word_count === 1 ? '' : 's'}</span>
              <span>📄 {[s.has_text1, s.has_text2].filter(Boolean).length} text{[s.has_text1, s.has_text2].filter(Boolean).length === 1 ? '' : 's'}</span>
            </div>
